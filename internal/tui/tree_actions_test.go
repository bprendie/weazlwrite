...
func TestTreeNCreatesVaultDocumentInSelectedFolder(t *testing.T) {
	...
	m = updated.(model)
	if !strings.HasPrefix(m.vaultPath, "projects/specs/untitled-") {
		t.Fatalf("vaultPath = %q, want projects/specs/untitled-*", m.vaultPath)
	}
	if m.dirty {
...
func TestTreeNCreatesFilesystemDocumentInSelectedFolder(t *testing.T) {
	...
	updated, _ := m.updateTree(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'n'}})
	m = updated.(model)
	if !strings.HasPrefix(m.diskPath, dir+string(filepath.Separator)+"untitled-") {
		t.Fatalf("diskPath = %q, want under %s", m.diskPath, dir)
	}
	if m.isVault {
